...
MMIORegion::MMIORegion(uintptr_t phys_addr, size_t size, memory::CacheType cache) : size(size) {
    ...
    for (size_t i = 0; i < this->mapped_size; i += PAGE_SIZE_4K) {
        if (!PageMap::get_kernel_map()->map(reinterpret_cast<uintptr_t>(this->page_base) + i,
                                            phys_page_start + i, Read | Write | Global, cache,
                                            PageSize::Size4K)) {
...
//     VirtualManager::free(this->page_base, this->size, PageSize::Size4K, false);
// }
