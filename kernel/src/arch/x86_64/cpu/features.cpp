...
bool check_feature(unsigned int leaf, int reg_idx, int bit) {
    ...
    if (!__get_cpuid(leaf, &eax, &ebx, &ecx, &edx)) {
        LOG_DEBUG("CPUID: leaf=0x%x unsupported; (reg=%d bit=%d) assumed absent",
                  leaf, reg_idx, bit);
        return false;
    }
...
bool check_feature(unsigned int leaf, unsigned int subleaf, int reg_idx, int bit) {
    ...
    if (!__get_cpuid_count(leaf, subleaf, &eax, &ebx, &ecx, &edx)) {
        LOG_DEBUG("CPUID: leaf=0x%x subleaf=0x%x unsupported; (reg=%d bit=%d) assumed absent",
                  leaf, subleaf, reg_idx, bit);
        return false;
    }
...
unsigned int get_cpuid_value(unsigned int leaf, unsigned int subleaf, int reg_idx) {
    ...
    if (!__get_cpuid_count(leaf, subleaf, &eax, &ebx, &ecx, &edx)) {
        ...
        LOG_DEBUG("CPUID: leaf=0x%x subleaf=0x%x unsupported; reg=%d -> 0",
